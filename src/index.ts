import {KnexpressoConfig, KnexpressoDependencies} from "./types/knexConfig.type";
...
export function startKnexpresso(config: KnexpressoConfig, port?: number, logger?: Logger) {
    ...
    return new Promise((resolve, reject) => {
        const server = app.listen(serverConfig.port, () => {
            KNEXPRESSO_LOGGER.info(`Knexpresso server started on port ${serverConfig.port}`);
