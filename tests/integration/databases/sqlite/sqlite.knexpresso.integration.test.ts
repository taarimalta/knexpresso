...
import { sqliteConfig } from '../../configs/sqlite.knexpressoConfig';
import {
    setupTestServer,
    teardownTestServer,
...
beforeAll(async () => {
    ({ server, db } = await setupTestServer(sqliteConfig));
});
...
describe('SQLite Integration Tests for Knexpresso API', () => {
    beforeEach(async () => {
